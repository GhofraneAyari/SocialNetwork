...
public class PageService {
    public void likePage() {
        try{
            ...
            System.out.println("---------------------------------------");
            System.out.println("Would you like to like a page? (Y/N)");
            Scanner input = new Scanner(System.in);
            String yesNo = input.nextLine();
            if(yesNo.equals("Y") || yesNo.equals("y") || yesNo.equals("yes") || yesNo.equals("YES"))
            {
                System.out.println("Enter the ID of the page that you would want to like...");
    ...
    public int addAdmin(int idUser, int idPage) throws SQLException {
        Connection cnx= SingletonDatabaseConnection.getInstance().cnx;
        try (Statement stmt1 = cnx.createStatement()) {
            return stmt1.executeUpdate("INSERT INTO pagemanager( pageId , userId ) " +
                    "VALUES(" + idPage + "," + idUser + ");");
        }

    }

    public void addOtherAdmin() throws SQLException {
        try {
    ...
    public int createPage(Page page) throws SQLException{
        Connection cnx= SingletonDatabaseConnection.getInstance().cnx;
        try (Statement stmt1 = cnx.createStatement()) {
            return stmt1.executeUpdate("INSERT INTO pages ( pageName, pageCreationDate, pageCreator, genre) " +
